$(function(){
  ...
  $("#playBtn").on("click",function(){
    $("#voiceDisplay").text(`${$("#myVideo")[0].volume.toFixed(1)*100}%`);
    $("#progressBar")[0].max = $("#myVideo")[0].duration;
   
    //console.log("yo"); 單純用來確認我的click有無作用
    ...
    if($("#myVideo")[0].paused){//判斷我的影片是否為暫停狀態(paused)
        ...
        $(".aa").text("pause");//將playbtn字改成stop
    }else{
        $("#myVideo")[0].pause();//反之 如果不是暫停狀態，點擊就暫停(pause)
        $(".aa").text("play_arrow");//然後改字樣成play
    }
  });
...
function downVolume(){//調整音量
    ...
    $("#voiceDisplay").text(`${myViedo.volume.toFixed(1)*100}%`);
...
function updateProgress(){
    ...
    $("#timeDisplay").append("/"+Math.floor($("#myVideo")[0].duration));//把秒以及總秒數加進去
...
//volume,paused,pause,webkitEnterFullscreen,duration都是viedo所內建的屬性